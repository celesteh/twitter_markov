...
def massage(tweet):
    ...
    tweet = re.sub('\s+[\:\.\,\-\;\?\!\"\'\#]+\s*\Z', '\n', tweet) # strip trailing punctuation
    tweet = re.sub('\s+\Z', '\n', tweet) # strip trailing whitespace
...
def hashtags(tweet):
    ...
    tweet = re.sub('\s+President\s*Trump', ' #PresidentTrump', tweet, flags=re.I)
...
def learn():
    ...
    terms = ['global warming', 'climate hoax', 'climate hustle', 'nuclear war', 'nuclear bomb', 'hydrogen bomb', 'nuclear missile', 'nuke', 'nuclear arms', 'proliferation', 'launch codes', '#MAGA', '#PresidentTrump']
    # 'climate change',
